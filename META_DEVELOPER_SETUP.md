...
在 Meta Developers 頁面頂部導航列中：

1. 找到並點擊「**應用程式**」（Apps）選單
   - 位置：頂部導航列，通常在「工具」（Tools）和「支援」（Support）之間
   - 如果看到下拉選單，點擊「應用程式」或「Apps」

2. 在應用程式頁面中：
   - 如果已有應用程式，會顯示應用程式列表
   - 如果沒有應用程式，會看到「建立應用程式」或「Create App」按鈕

### 1.3 建立新 App（新版系統）

#### 方法一：從應用程式頁面建立

1. 點擊「**建立應用程式**」或「**Create App**」按鈕
...
3. 填寫應用程式詳細資料：
   - **應用程式名稱**: `土城貓舍自動發文系統`（或您喜歡的名稱，最多 30 字元）
...
如果找不到「應用程式」選單，可以直接前往：

1. 前往：https://developers.facebook.com/apps/creation/
2. 按照上述步驟 3-5 完成建立
